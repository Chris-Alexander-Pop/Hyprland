...
| `input: hold client pointer over a named layer` | Optional. A named layer-shell namespace does not `pointer.leave` the window below. Motion is forwarded. `misc:layer_hold_pointer` / `misc:layer_hold_pointer_namespace`. |
