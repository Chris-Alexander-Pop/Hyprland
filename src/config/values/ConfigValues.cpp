...
#define MS makeConfigValue
    ...
    return std::vector<SP<IValue>>{
        ...
        MS<Bool>("misc:layer_hold_freeze",
                 "While layer_hold_pointer is on, do not deliver pointer events to the surface below the held layer. Restore the cursor position when unset.", false,
                 {.refresh = Supplementary::REFRESH_LAYER_HOLD_FREEZE}),
